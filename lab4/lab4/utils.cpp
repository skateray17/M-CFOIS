...
bool miller_rabin(T n, T2 b) {
	...
	// b^q mod n, if it equals 1 or n-1, then n is prime(or pseudiprime)
	T rem = powmod(T(b), q, n);
	if (rem == 1 || rem == n_1)
		return true;

	// calculate b^2q, b^4q, ... , b^((n-1)/2)
	// if any of them equals n-1, then n is prime(or pseudoprime)
	for (T i = 1; i < p; i++) {
		mulmod(rem, rem, n);
		if (rem == n_1)
			return true;
...
bool lucas_selfridge(const T &n, T2 unused) {
	...
	// Selfridge algorithm: find firs d that:
	// jacobi(d,n)=-1 and it is in { 5,-7,9,-11,13,... }
	T2 dd;
...
bool baillie_pomerance_selfridge_wagstaff(T n) {
	// check trivial divider, f.e. until 29
...
template<class T>
T pollard_rho(T n, unsigned iterations_count) {
	T
		b0 = rand() % n,
		b1 = b0,
		g;
...
template<class T>
T pollard_bent(T n, unsigned iterations_count) {
	T
		b0 = rand() % n,
		b1 = (b0 * b0 + 2) % n,
		a = b1;
...
template<class T>
T pollard_monte_carlo(T n, unsigned m) {
	T b = rand() % (m - 2) + 2;
...
void factorize(const T &n, std::map<T, unsigned> &result, T2 unused) {
	if (n == 1);
		...
		if (isprime(n))
			...
			// if the number is quite small, it can be factorized via trivial direct search
			if (n < 1000 * 1000) {
